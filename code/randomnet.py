...
# returns a net with a random number of nodes
...
def randomtrainingset(net, weight_l, weight_u, bias_l, bias_u, input_l, input_u, output_l, output_u):
    ...
    weights_list = [trunc(random.uniform(weight_l, weight_u)) for x in xrange(weights_needed)]
    
    randomset.weights = weights_list

    # compute the number of biases needed
    biases_needed = net.numHidden + net.numOutput
    biases_list = [trunc(random.uniform(bias_l, bias_u)) for x in xrange(biases_needed)]    
    ...
    # get inputs
    inputs = [trunc(random.uniform(input_l, input_u)) for x in xrange(net.numInputs)]
    print "Inputs: %s\n" % inputs
    randomset.inputs = inputs
    
    # get outputs
    outputs = [trunc(random.uniform(output_l, output_u)) for x in xrange(net.numOutput)]
    print "Outputs: %s\n" % outputs 
